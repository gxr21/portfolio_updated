...
function LandingPage() {
  ...
  return <><NavBar /><main className="min-h-screen bg-surface" dir={direction}>
    <motion.section id="home" variants={scrollReveal} initial="hidden" whileInView="visible" viewport={scrollViewport} className="container mx-auto flex flex-col items-center justify-between px-4 py-20 scroll-mt-20 md:flex-row">
      <div className="max-w-2xl"><h1 className="mb-4 text-6xl font-bold text-black">{t.landing.name}</h1><h2 className="text-2xl font-bold text-black">{t.landing.role}</h2><p className="mt-4 text-lg leading-8 text-gray-600">{t.landing.intro}</p><a href="/uplod/Ali-Jalal-CV.pdf" download="Ali-Jalal-CV.pdf" className="mt-6 inline-flex h-12 w-44 items-center justify-center rounded-md border border-primary text-primary transition-colors hover:bg-primary hover:text-white">{t.landing.downloadCv}</a></div>
