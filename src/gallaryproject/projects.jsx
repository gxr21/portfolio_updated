  ...
  return <main className="min-h-screen bg-surface" dir={direction}>
    ...
    <motion.section variants={scrollReveal} initial="hidden" whileInView="visible" viewport={scrollViewport} className="container mx-auto px-4 py-20">
      ...
      <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={scrollViewport} className="mt-8 grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
        {t.projects.items.map((project, index) =><motion.article variants={cardReveal} key={`project-${index}`} className="overflow-hidden rounded-lg border bg-white shadow transition-shadow hover:shadow-lg">
        <img src={images[index]} alt="" className="h-48 w-full object-cover" loading="lazy" />
        <div className="p-6">
          <h2 className="text-xl font-bold">{project.title}</h2>
        <p className="mt-4 leading-7 text-gray-600">{project.description}</p></div></motion.article>)}
        </motion.div>
        </motion.section>
